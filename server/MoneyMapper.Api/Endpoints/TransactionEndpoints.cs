    ...
    public static RouteGroupBuilder MapTransactionEndpoints(this IEndpointRouteBuilder app)
    {
        ...
        transactions.MapGet(
            ...
            async (Guid id, MoneyMapperDbContext dbContext) =>
            {
                ...
                return transaction is null ? Results.NotFound() : Results.Ok(transaction.ToDto());
            }
        );

        transactions.MapPost(
            "/",
            async (CreateTransactionRequest request, MoneyMapperDbContext dbContext) =>
            {
                ...
                var transaction = new Transaction
                {
                    ...
                    Type = request.Type,
                    ExpenseType = request.Type == "expense" ? request.ExpenseType : null,
        ...
        transactions.MapPut(
            "/{id:guid}",
            async (Guid id, UpdateTransactionRequest request, MoneyMapperDbContext dbContext) =>
            {
                var errors = TransactionValidator.Validate(request);
                ...
                transaction.ExpenseType = request.Type == "expense" ? request.ExpenseType : null;
